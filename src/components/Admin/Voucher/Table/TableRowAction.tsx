...
import { deleteVoucherByIdAction } from "@/lib/actions/voucherAction";
...
export function TableRowAction<TData>({ row }: TableRowActionProps<TData>) {
  const voucher = VoucherSchema.parse(row.original);
  ...
  const onDeleteUserHandler = useCallback(
    async (id: string) => {
      setIsLoading(true);
      const { status, message } = await deleteVoucherByIdAction(id);
      if (status !== "success") {
        toast.error(message);
        setIsLoading(false);
        return;
  ...
  return (
    <Dialog open={openDialog} onOpenChange={setOpenDialog}>
      ...
      <DialogContent className="bg-white max-w-[350px] md:min-w-[350px] md:max-w-md rounded overflow-auto max-h-[95dvh]">
        {statusDialog === "edit" ? (
          <FormEditVoucher voucher={voucher} setOpenDialog={setOpenDialog} />
        ) : (
          <div className="flex flex-col gap-5 items-center justify-center">
            <h1 className="text-xl">Yakin ingin menghapus voucher ini?</h1>
            <div className="flex gap-2 justify-center">
              ...
              <Button
                ...
                variant={"primary"}
                disabled={isLoading}
                onClick={() => onDeleteUserHandler(voucher.id)}
              >
                {isLoading ? (
                  <>
